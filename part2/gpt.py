...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return x / torch.sqrt(torch.mean(torch.square(x), dim=-1, keepdim=True) + self.eps) * self.weight
...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        # NOTE: I assume the embedding dimension is an even number
        dim = xq.shape[-1]

        # Generate RoPE embeddings dynamically based on T
        seq_pos = torch.arange(0, T, device=self.mask.device) # Shape: (T)
        freqs = self.inv_freq.to(self.mask.device).expand(T, -1) # Shape: (T, dim // 2)

        pos_emb = torch.repeat_interleave(
            seq_pos[:, None] * freqs, 2, dim=-1
        ).view(1, 1, T, dim) # Shape: (1, 1, T, dim)
        
        # Split pos into sin and cos components, repeating each to match xq and xk dimensions
        pos_sin = torch.sin(pos_emb).expand_as(xq)
        pos_cos = torch.cos(pos_emb).expand_as(xq)
        
        # Apply RoPE transformation: pair and rotate dimensions
        # Rotate query and key tensors
        xq_rot = xq * pos_cos
        # even and uneven indices are treated differently
        xq_rot[:, :, :, ::2] -= xq[:, :, :, 1::2] * pos_sin[:, :, :, ::2]
        xq_rot[:, :, :, 1::2] += xq[:, :, :, ::2] * pos_sin[:, :, :, ::2]

        xk_rot = xk * pos_cos
        xk_rot[:, :, :, ::2] -= xk[:, :, :, 1::2] * pos_sin[:, :, :, ::2]
    ...
    def forward(self, x):
        ...
        # apply linear layers and view shape [B, T, 3C] as [B, nh, T, 3*hs]
        chunks = self.c_attn(x).view(B, T, self.n_head, -1).transpose(1, 2)
        # split shape [B, nh, T, 3*hs] into [3, B, nh, T, hs]
        q, k, v = torch.chunk(chunks, 3, dim=-1)
...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mlpf = nn.Sequential(
            ...
            nn.Dropout(config.resid_pdrop) # TODO: is this correct?
        )
...
class GPT(nn.Module):
    ...
    def generate(self, idx: torch.LongTensor, max_new_tokens: int, temperature:float = 1.0, do_sample:bool = False, top_k:int = None, top_p: float = 0.6):
        ...
        for _ in range(max_new_tokens):
            ...
            logits = torch.squeeze(self.forward(idx_cond)[:, -1, :], dim=1) / temperature # shape: (batch_size, vocabulary_size)
            
            if not do_sample: # TODO:
                # take the most likely token
                idx_next = torch.argmax(logits, dim=-1)[:, None] # shape: (batch_size, 1)
            else:
                # apply softmax to convert logits to (normalized) probabilities
                logits = F.softmax(logits, dim=-1)
                row_idx = torch.arange(logits.size(0)).unsqueeze(1)

                # optionally only consider top-k logits for sampling. 
                if top_k is not None:
                    probs, inds = torch.topk(logits, top_k, dim=-1)
                    sampled_idx = torch.multinomial(probs, 1)
                    idx_next = inds[row_idx, sampled_idx] # shape: (batch_size, 1)
                ...
                if top_p is not None:
                    ...
                    # shift to 1 to right so it includes the values necessary to exceed 0.6
                    prob_mask = torch.roll(prob_mask, 1, dims=-1)
                    prob_mask[:, 0] = True

                    # unsort mask to match logits
                    prob_mask[row_idx, sort_idx] = prob_mask.clone()
                    # mask logits
                    idx_next = torch.multinomial(logits * prob_mask, 1)
